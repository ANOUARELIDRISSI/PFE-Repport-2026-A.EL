...
During the end-of-study internship, this logic was applied through Trello (see below): user stories and decomposed tasks (for example ingestion, graph publication, or reasoning-mode integration) were tracked as cards moving through analogous states. A story was treated as complete only when the associated module produced the evidence artifacts expected for its requirement area (logs, indexed documents, graph edges, traces, or dashboards), which kept validation continuous rather than deferred to a single final gate.

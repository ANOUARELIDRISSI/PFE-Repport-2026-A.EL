...
The objective is to measure the complete behavior of the Graph-RAG
architecture, including evidence retrieval, graph-supported reasoning,
cited answer generation, knowledge extraction during ingestion, and
...
answer. This complete protocol ensures that the reported results reflect the
...
on chronology and claims that depend on time; and \textbf{comparative} questions
...
Temporal & 35 & Chronology, validity intervals, and claims that depend on time. \\
Comparative & 35 & Multi-source comparison of actors, policies, or positions. \\
\midrule
Total & 150 & Balanced coverage of direct and multi-step political analysis. \\
\bottomrule
\end{tabularx}
...
\begin{figure}[H]
    ...
    \caption{Evaluation pipeline from benchmark questions to aggregated results by mode.}
...
The results by mode establish the effect of reasoning depth; the next
...
\textbf{Comparison before and after DSPy optimization.}
...
Ground truth & Wealthier economies mainly face inflation and pressure on energy prices, whereas vulnerable Global South economies face stronger import, fiscal, and food security constraints. \\
Retrieved context & Six ranked chunks concerning inflation in fuel prices, import dependence, fiscal space, and regional economic vulnerability, supplemented by two geopolitical facts linked through the graph. \\
...
FR-03 & Graph storage and entity resolution & Canonical identifiers, typed Neo4j nodes and relations, and repeatable merge behavior were verified during ingestion. & Validated \\
FR-04 & Hybrid vector and graph retrieval & ZVec chunks and Neo4j facts were recorded together in query evidence and evaluated across 150 questions. & Validated \\
FR-05 & Selectable reasoning modes & RAG, CoT, and reflexive modes evaluated through 450 query executions. & Validated \\
FR-06 & Temporal traceability & Temporal metadata and the dedicated temporal benchmark category were inspected during evaluation. & Validated \\
...
FR-08 & Autonomous discovery & Scheduled registry collection and administrator-triggered topic discovery were exercised through the ingestion workflow. & Validated \\
FR-09 & Domain-independent ingestion & Acquisition and normalization operated separately from domain-specific extraction modules. & Validated \\
FR-10 & Heterogeneous normalization & RSS, HTML, direct URL, and PDF inputs produced the unified document representation defined in Chapter~3. & Validated \\
FR-11 & Automated validation & The offline evaluation harness applied repeatable LLM-assisted extraction and response scoring. & Validated \\
...
\textbf{Threats to validity.} Four limitations bound the conclusions. \emph{Corpus coverage:} scores depend on what was ingested at evaluation time. \emph{Judge bias:} scoring based on LLMs may favor fluent answers or penalize careful abstention. \emph{Temporal drift:} political facts change; the 150-question snapshot does not guarantee stable scores after new ingestion. \emph{Benchmark coverage:} although the dataset supports comparison by mode, it cannot represent every geopolitical region, language, event type, or analytical formulation; additional multilingual questions and human analyst ratings would strengthen external validity.

\section*{Conclusion}

This chapter evaluated the platform through a benchmark comprising
\textbf{150 questions} and \textbf{450 executions} across three reasoning
modes. The assessment combined RAGAS-based metrics, embedding comparisons,
extraction experiments, and requirements validation. The results indicate
that greater reasoning depth improves answer quality and evidence coverage,
although it introduces additional latency. Furthermore, Mistral embeddings
improved retrieval performance, while DSPy optimization increased extraction
reliability and reduced processing time. Overall, these findings support the
technical validity of the proposed Graph-RAG approach while identifying
retrieval completeness, multilingual coverage, and temporal consistency as

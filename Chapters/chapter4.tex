...
\textbf{Chunking and context expansion.} Documents are indexed as paragraph-aware chunks rather than as complete files. Neighboring chunks retain document identifiers, sequence positions, section headers, source metadata, and entity links. This structure limits the risk that a fixed boundary separates an actor from an action or a claim from its date. When a high-scoring chunk lacks sufficient local context, the retrieval layer can recover adjacent segments from the same document before synthesis. The first search therefore remains precise over compact vectors, while selective expansion improves recall around promising evidence instead of filling the model context with an entire source document. Recursive and structure-aware splitting patterns documented by LangChain and node-based parsing concepts documented by LlamaIndex provide practical reference points for this implementation strategy \cite{langchain-splitters,llamaindex-node-parsers}.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth,height=0.82\textheight,keepaspectratio]{images/zvec Performance at Scale.png}

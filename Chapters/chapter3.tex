...
The ingestion engine combines three complementary levels rather than relying on a single crawler template. \textbf{Level~1} monitors a curated registry of \textbf{57 high-precision outlets} according to predefined schedules (RSS feeds, institutional portals, and established political sources). \textbf{Level~2} extends coverage through query-driven discovery of thematic gaps, including search, link traversal, dynamic rendering, and pagination. \textbf{Level~3} processes unstructured documents, such as PDF reports, using high-fidelity parsers that preserve layouts and tables. Figure~\ref{fig:autonomous-crawler-final} details the crawler control flow. A source registry and scheduler populate a prioritized URL frontier; acquisition policies enforce robots directives, rate limits, and retry rules; and a content router selects lightweight extraction or browser-assisted acquisition according to the response. Both paths converge on a unified document representation, quality gates, MongoDB persistence, and downstream knowledge processing. Failed acquisitions enter a bounded retry and manual-review process rather than silently contaminating the corpus.

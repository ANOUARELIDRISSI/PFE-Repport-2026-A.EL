...
Joint entity and relation extraction is orchestrated through DSPy modules that call Mistral in JSON mode. Prompt templates are treated as optimizable modules rather than static strings. The MIPROv2 Bayesian optimizer evaluates alternative instructions and few-shot examples on controlled batches and retains the configuration that maximizes a composite extraction score. The project benchmark reports a relative improvement of approximately \textbf{22.1\%} in the composite score and a reduction of \textbf{1.41\,s} in the average inference time per document compared with the initial manual prompt. Offline DSPy optimization is therefore separated from online serving through the production Mistral client, thereby preventing prompt iteration from destabilizing the live API.
...
\textbf{Chunking and context expansion.} Documents are indexed as paragraph-aware chunks rather than as complete files. Neighboring chunks retain document identifiers, sequence positions, section headings, source metadata, and entity links. This structure reduces the risk that a fixed boundary will separate an actor from an action or a claim from its date. When a highly ranked chunk lacks sufficient local context, the retrieval layer can recover adjacent segments from the same document before synthesis. Initial retrieval therefore remains precise over compact vectors, while selective expansion improves recall around relevant evidence without filling the model context with an entire source document. Recursive splitting patterns documented by LangChain and node-based parsing concepts documented by LlamaIndex provide practical reference points for this implementation strategy \cite{langchain-splitters,llamaindex-node-parsers}.
...
Hybrid ranking combines semantic scores from ZVec, graph distance to seed entities, temporal validity on edges, and deduplication of syndicated passages. Synthesis prompts require explicit source references: chunk identifiers, document titles, and graph fact summaries are passed as structured context. The FastAPI \texttt{/query} route streams reasoning steps to the client using server-sent events, enabling the chat interface to present the planning, retrieval, audit, and synthesis phases in real time. Chapter~5 evaluates answer quality and grounding across modes on the same deployed endpoint.
...
Figure~\ref{fig:ui-chat-rag} illustrates a completed answer in RAG mode: the assistant message includes inline citations, source cards, and confidence cues tied to retrieved chunks. Figure~\ref{fig:ui-chat-rag-trace} expands the reasoning trace panel and presents the sequential ZVec retrieval and synthesis steps. This representation allows users to verify the evidence supporting each claim and implements FR-07 at the interface level.
...
This chapter presented the engineering realization of the proposed
architecture. The implementation integrates DSPy-optimized extraction,
MongoDB document storage, Neo4j graph persistence, ZVec semantic retrieval,
three reasoning modes, external web supplementation, a React analytical
interface, and a containerized observability environment. Collectively, these
components form an operational platform for evidence acquisition, knowledge
construction, hybrid retrieval, and cited answer generation. Nevertheless,
technical completion alone does not establish the effectiveness or production
readiness of the system. This raises a central question: to what extent can
the implemented platform satisfy the established requirements in terms of
accuracy, latency, reliability, scalability, and evidence quality under
